 ...
 * <i> Peekable stores the next value in the private {@code nextElement} field, so if your iterator
 * has any side effects, you should be aware of the fact that the {@link Iterator#next} method is
 * invoked firstly in the constructor and then for each subsequent {@link Peekable#next} call, the
 * value from the private field is returned and replaced with the one returned from the new
